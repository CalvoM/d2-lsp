...
func (s *StateManager) GoToDefinition(uri DocumentURI, position Position) Location {
	...
	LspLOG.Printf("The ident is found at %v and %v. (%v)", start, end, doc.Text[start:end])
	s.convertBytePositionToLocation(doc.Text, position.Line, start)
	s.convertBytePositionToLocation(doc.Text, position.Line, end)
	return Location{}
}
...
func (s *StateManager) convertBytePositionToLocation(text string, row int, bytePosition uint) {
	lines := strings.Split(text, "\n")
	byteCount := 0
	for line_index, line := range lines {
		if line_index == row {
	...
	LspLOG.Printf("Col found is %v", col)
}
